...
export default function App() {
  ...
  // Cek status login
  useEffect(() => {
    fetch(`${import.meta.env.VITE_BACKEND_URL}/auth/me`, { credentials: "include" }) //fetch("http://localhost:3000/auth/me", { credentials: "include" })
    .then((r) => r.json())
  ...
  useEffect(() => {
    if (!loggedIn) return
    fetch(`${import.meta.env.VITE_BACKEND_URL}/classroom/courses`, { credentials: "include" })//fetch("http://localhost:3000/classroom/courses", { credentials: "include" })
      .then((r) => r.json())
  ...
  const loadSubmissions = async (courseId: string) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/classroom/courses/${courseId}/submissions`, //`http://localhost:3000/classroom/courses/${courseId}/submissions`,
        { credentials: "include" }
      )
  ...
  const handleLogin = () => {
    window.location.href = `${import.meta.env.VITE_BACKEND_URL}/auth/login` //window.location.href = "http://localhost:3000/auth/login"
  }

  const handleLogout = async () => {
    await fetch(`${import.meta.env.VITE_BACKEND_URL}/auth/logout`, { method: "POST", credentials: "include" }) //await fetch("http://localhost:3000/auth/logout", { method: "POST", credentials: "include" })
    setLoggedIn(false)
    setCourses([])
    setItems([])
